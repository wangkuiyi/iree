...
const iree_hal_metal_device_params_t* iree_hal_metal_driver_device_params(
  ...
  const iree_hal_metal_driver_t* driver = iree_hal_metal_driver_const_cast(base_driver);
  return &driver->device_params;
}
...
static iree_status_t iree_hal_metal_populate_device_info(id<MTLDevice> device, uint8_t* buffer_ptr,
  ...
  *out_buffer_ptr = buffer_ptr;
  ...
  const int name_len = strlen(device_name);
...
static iree_status_t iree_hal_metal_driver_create_device_by_id(iree_hal_driver_t* base_driver,
  ...
  iree_status_t status = iree_hal_metal_device_create(
      base_driver, device_name, &driver->device_params, device, host_allocator, out_device);
...
static iree_status_t iree_hal_metal_driver_create_device_by_path(
  ...
  if (iree_string_view_parse_hex_bytes(device_path, 8, device_registry_id)) {
